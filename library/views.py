...
from library.models import Artist, RecordTitle, Country, FormatType, ReleaseYear, RecordLabel, RecordReview, \
    CatalogNumber, IssueNumber, FileUnder, Notes, ReviewerName, ReviewerName, BandMember
import json
...
def artist(request, artist_id):
    ...
    try:
        ...
        print "No Artist found with ID" + artist_id
        pass
    record_title_list = RecordTitle.objects.filter(artists__id=artist_id)
    # Create a context dictionary which we can pass to the template rendering engine.
    # We start by containing the name of the artist passed by the user.
    # The artist detail view will also display the title, country and file_under fields.
    context_dict = {
...
def record_label_detail(request, record_label_id):
    ...
    try:
        ...
        record_title_list = RecordTitle.objects.filter(record_labels__id=record_label_id)
    except RecordTitle.DoesNotExist:
        pass
...
def country_detail(request, country_id):
    ...
    try:
        ...
        artist_list = Artist.objects.filter(country__id=country_id)
    except Artist.DoesNotExist:
        pass
...
def record_reviewer_detail(request, reviewer_name_id):
    ...
    try:
        ...
        record_title_list = RecordTitle.objects.filter(reviewer_names__id=reviewer_name_id)
    except RecordTitle.DoesNotExist:
        pass
...
def issue_number_detail(request, issue_number_id):
    context = RequestContext(request)
    try:
        issue_number = IssueNumber.objects.get(id=issue_number_id)
...
def band_member_detail(request, band_member_id):
    context = RequestContext(request)
    try:
        band_member = BandMember.objects.get(id=int(band_member_id))
    except BandMember.DoesNotExist:
        pass
    try:
        # filters record title data to appropriate record label
        artist_list = Artist.objects.filter(band_members__id=band_member_id)
    except Artist.DoesNotExist:
        pass
    try:
        record_title_list = RecordTitle.objects.filter(band_members__id=band_member_id)
    except RecordTitle.DoesNotExist:
        pass
    ...
    return render_to_response('library/band_member_detail.html', context_dict, context)
...
#     with open("/home/ubuntu/mrr-archive-django/record_collection_5.json") as f:
